...
TypeId CustomApplication::GetInstanceTypeId(void) const
{
...
void
CustomApplication::Configure(void) {
...
void
CustomApplication::StartApplicationWithData(void)
{
  Configure();
  if (m_waveDevice)
  {
      //Let's create a bit of randomness with the first broadcast packet time to avoid collision
        Time random_offset = MicroSeconds (m_random->GetValue(50,200));
...
void
CustomApplication::SetPacketSize (uint32_t packetSize)
{
